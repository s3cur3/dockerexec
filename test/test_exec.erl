...
run(Count, Timeout, DelayMS) when is_integer(Count), is_integer(Timeout), is_integer(DelayMS) ->
    %application:ensure_started(dockerexec),
    ...
    receive
        ...
        {completed, Pid, IOs, Success, FdErr} when FdErr > 0 ->
            {error, hit_limit_of_max_open_files, [
                {io_ops, IOs}, {success, Success}, {fd_err, FdErr}
            ]};
...
init([Owner, Count, DelayMS]) ->
    ...
    Cmd = <<
        "echo \"Test stdout $$\"; echo \"Test stderr $$\" 1>&2; "
        "sleep $[ ((($RANDOM % ",
        Delay/binary,
        ") + 1)/1000) ]; "
        "exit 12\n"
...
handle_info(
    start_child,
    #state{count = Count, active = Active, cmd = Cmd, fderr = Err, pids = Pids} = State0
) ->
    {I, State} =
        case dockerexec:run(Cmd, [stdout, stderr, monitor]) of
            {ok, _Pid, OsPid} ->
                {10, State0#state{
                    count = Count - 1, active = Active + 1, pids = sets:add_element(OsPid, Pids)
                }};
...
handle_info({stderr, _OsPid, <<"Test stderr ", _Rest/binary>>}, State) ->
    {noreply, State#state{ios = State#state.ios + 1}};
handle_info(
    {'DOWN', OsPid, process, _Pid, {exit_status, ExitStatus}},
    #state{pids = Pids, success = N} = State
) ->
    case sets:is_element(OsPid, Pids) of
        true ->
            ...
            Success =
                case dockerexec:status(ExitStatus) of
                    {status, 12} -> N + 1;
                    _ -> N
                end,
