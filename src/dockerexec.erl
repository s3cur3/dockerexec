...
%%%      `exec.cpp' file.  The `dockerexec' application can execute tasks by
...
%%%      </ul>
%%%      In either of these two cases, `dockerexec:start_link/2' must be started
...
%%%      `dockerexec': `[root, {user, "root"}, {limit_users, ["root"]}]'.
%%%
...
-record(state, {
    port,
    % Last transaction number sent to port
    last_trans = 0,
    % Queue of outstanding transactions sent to port
    trans = queue:new(),
    % Restricted list of users allowed to run commands
    limit_users = [],
    % Pids to notify when an OsPid exits
    registry,
    debug = false,
    root = false
}).
...
%% Options passed to the dockerexec process at startup. They can be specified in the
%% `sys.config' file for the `dockerexec' application to customize application
%% startup.
%% <dl>
...
%%  2> dockerexec(Filename, []) % Argh!!! This is not good!
%% '''
%%
...
%%         input to the process is sent by `dockerexec:send(OsPid, Data)'.
...
run(Exe, Options, Timeout) when
    (is_binary(Exe) orelse is_list(Exe)) andalso
        is_list(Options) andalso is_integer(Timeout)
->
...
-spec run_link(cmd(), cmd_options(), integer()) ->
    {ok, pid(), ospid()} | {ok, [{stdout | stderr, [binary()]}]} | {error, any()}.
run_link(Exe, Options, Timeout) when
    (is_binary(Exe) orelse is_list(Exe)) andalso
        is_list(Options) andalso is_integer(Timeout)
->
...
%%      would be managed by dockerexec.
%% @end
%%-------------------------------------------------------------------------
...
kill(Pid, Signal) when
    (is_pid(Pid) orelse is_integer(Pid)) andalso
        is_integer(Signal)
->
...
send(OsPid, Data) when
    (is_integer(OsPid) orelse is_pid(OsPid)),
...
default() ->
    % Debug mode of the port program.
    [
        {debug, 0},
        % Verbose print of events on the Erlang side.
        {verbose, false},
        % Allow running processes as root.
        {root, false},
        % Extra arguments that can be passed to port program
        {args, ""},
        {alarm, 12},
        {portexe, noportexe},
        % Run port program as this user
        {user, ""},
        % Restricted list of users allowed to run commands
        {limit_users, []}
    ].
...
default(portexe) ->
    ...
    case code:priv_dir(dockerexec) of
        {error, _} ->
...
init([Options]) ->
    ...
    Exe0 =
        case proplists:get_value(portexe, Options, noportexe) of
    ...
    Users =
        case proplists:get_value(limit_users, Options, default(limit_users)) of
            [] -> [];
            L -> [to_list(I) || I <- L]
        end,
    ...
    Exe =
        if
            ...
            not IsRoot,
            NeedSudo,
            ((User /= undefined andalso User /= "" andalso User /= EffUsr) orelse
                (EffUsr /= User andalso User /= undefined andalso
                    User /= root andalso
                    User /= "root")) ->
    ...
    debug(
        ...
        [
            ...
            if
                (Root orelse IsRoot) andalso User =:= [] -> "[ROOT] ";
                true -> ""
            end,
            Exe,
            if
    ...
    try
        ...
        after 350 ->
            ...
            {ok, #state{
                port = Port, limit_users = Users, debug = Debug, registry = Tab, root = Root
            }}
        end
    catch
...
handle_call({port, Instruction}, From, #state{last_trans = Last} = State) ->
    try is_port_command(Instruction, element(1, From), State) of
        ...
        {ok, Term, Link, Sync, PidOpts} ->
            ...
            {noreply, State#state{
                trans = queue:in({Next, From, Link, Sync, PidOpts}, State#state.trans)
            }}
    catch
        _:{error, Why} ->
...
handle_info({Port, {data, Bin}}, #state{port = Port, debug = Debug} = State) ->
    ...
    case Msg of
        ...
        {0, {exit_status, OsPid, Status}} ->
            debug(
                Debug,
                "Pid ~w exited with status: ~s{~w,~w}\n",
                [
                    OsPid,
                    if
                        (((Status band 16#7F) + 1) bsr 1) > 0 -> "signaled ";
                        true -> ""
                    end,
                    (Status band 16#FF00 bsr 8),
                    Status band 127
                ]
            ),
        ...
        case wait_port_exit(State#state.port) of
            ...
            S ->
                error_logger:warning_msg(
                    "~w - dockerexec process terminated (status: ~w)\n",
                    [self(), S]
                )
        end
    catch
...
ospid_loop({Pid, OsPid, Parent, StdOut, StdErr, IsMon, Debug} = State) ->
    receive
        ...
        {'DOWN', OsPid, {exit_status, Status}} ->
            debug(
                Debug,
                "~w ~w got down message (~w) (ismon=~w)\n",
...
is_suid_and_root_owner(File) ->
    case file:read_file_info(File) of
        {ok, Info} ->
            {(Info#file_info.mode band 8#4500) =:= 8#4500, (Info#file_info.uid =/= 0)};
        {error, Err} ->
...
is_port_command({send, Pid, Data}, _Pid, _State) when
    is_pid(Pid), is_binary(Data) orelse Data =:= eof
->
    case ets:lookup(exec_mon, Pid) of
        [{Pid, OsPid}] -> {ok, {stdin, OsPid, Data}};
        [] -> throw({error, no_process})
    end;
is_port_command({send, OsPid, Data}, _Pid, _State) when
    is_integer(OsPid), is_binary(Data) orelse Data =:= eof
->
    {ok, {stdin, OsPid, Data}};
is_port_command({winsz, Pid, Rows, Cols}, _Pid, _State) when
    is_pid(Pid), is_integer(Rows), is_integer(Cols)
->
    case ets:lookup(exec_mon, Pid) of
        [{Pid, OsPid}] -> {ok, {winsz, OsPid, Rows, Cols}, undefined, undefined, []};
        [] -> throw({error, no_process})
    end;
is_port_command({winsz, OsPid, Rows, Cols}, _Pid, _State) when
    is_integer(OsPid), is_integer(Rows), is_integer(Cols)
->
    {ok, {winsz, OsPid, Rows, Cols}, undefined, undefined, []};
is_port_command({pty_opts, Pid, Opts}, _Pid, _State) when
    is_pid(Pid), is_list(Opts)
->
...
is_port_command({pty_opts, OsPid, Opts}, _Pid, _State) when
    is_integer(OsPid), is_list(Opts)
->
    ok = check_pty_opts(Opts),
...
parse_env([]) -> [];
%% Remove the env var K
parse_env([{K, false} | T]) -> [{to_list(K), false} | parse_env(T)];
...
check_cmd_options([link | T], Pid, State, PortOpts, OtherOpts) ->
    check_cmd_options(T, Pid, State, PortOpts, OtherOpts);
check_cmd_options([{executable, V} = H | T], Pid, State, PortOpts, OtherOpts) when
    is_list(V); is_binary(V)
->
    check_cmd_options(T, Pid, State, [H | PortOpts], OtherOpts);
check_cmd_options([{cd, Dir} = H | T], Pid, State, PortOpts, OtherOpts) when
    is_list(Dir); is_binary(Dir)
->
    check_cmd_options(T, Pid, State, [H | PortOpts], OtherOpts);
check_cmd_options([{env, Env} = H | T], Pid, State, PortOpts, OtherOpts) when is_list(Env) ->
    case
        lists:filter(
            fun
                (S) when is_list(S); is_binary(S) -> false;
                ({S1, S2}) when
                    (is_list(S1) orelse is_binary(S1)) andalso
                        (is_list(S2) orelse is_binary(S2) orelse S2 == false)
                ->
                    false;
                (clear) ->
                    false;
...
check_cmd_options([{kill, Cmd} = H | T], Pid, State, PortOpts, OtherOpts) when
    is_list(Cmd); is_binary(Cmd)
->
    check_cmd_options(T, Pid, State, [H | PortOpts], OtherOpts);
check_cmd_options([{kill_timeout, I} = H | T], Pid, State, PortOpts, OtherOpts) when
    is_integer(I), I >= 0
->
    check_cmd_options(T, Pid, State, [H | PortOpts], OtherOpts);
check_cmd_options([kill_group = H | T], Pid, State, PortOpts, OtherOpts) ->
    check_cmd_options(T, Pid, State, [H | PortOpts], OtherOpts);
check_cmd_options([{nice, I} = H | T], Pid, State, PortOpts, OtherOpts) when
    is_integer(I), I >= -20, I =< 20
->
    check_cmd_options(T, Pid, State, [H | PortOpts], OtherOpts);
check_cmd_options([debug | T], Pid, State, PortOpts, OtherOpts) ->
    check_cmd_options(T, Pid, State, [{debug, 1} | PortOpts], OtherOpts);
check_cmd_options([{debug, I} = H | T], Pid, State, PortOpts, OtherOpts) when
    is_integer(I), I >= 0, I =< 10
->
    check_cmd_options(T, Pid, State, [H | PortOpts], OtherOpts);
check_cmd_options([{success_exit_code, I} = H | T], Pid, State, PortOpts, OtherOpts) when
    is_integer(I), I >= 0, I < 256
->
    check_cmd_options(T, Pid, State, [H | PortOpts], OtherOpts);
check_cmd_options([H | T], Pid, State, PortOpts, OtherOpts) when
    H =:= stdin; H =:= stdout; H =:= stderr
->
...
check_cmd_options([H | T], Pid, State, PortOpts, OtherOpts) when H =:= pty_echo ->
    check_cmd_options(T, Pid, State, [H | PortOpts], [{H, Pid} | OtherOpts]);
check_cmd_options([{winsz, {Rows, Cols}} = H | T], Pid, State, PortOpts, OtherOpts) when
    is_integer(Rows), Rows >= 0, is_integer(Cols), Cols >= 0
->
    check_cmd_options(T, Pid, State, [H | PortOpts], [{H, Pid} | OtherOpts]);
check_cmd_options([{pty, Pty} = H | T], Pid, State, PortOpts, OtherOpts) when is_list(Pty) ->
    ok = check_pty_opts(Pty),
    check_cmd_options(T, Pid, State, [H | PortOpts], OtherOpts);
check_cmd_options([{stdin, I} = H | T], Pid, State, PortOpts, OtherOpts) when
    I =:= null; I =:= close; is_list(I); is_binary(I)
->
    check_cmd_options(T, Pid, State, [H | PortOpts], OtherOpts);
check_cmd_options([{Std, I, Opts} = H | T], Pid, State, PortOpts, OtherOpts) when
    (Std =:= stdout orelse Std =:= stderr) andalso (is_list(Opts) orelse is_binary(Opts))
->
    ...
    check_cmd_options(T, Pid, State, [H | PortOpts], OtherOpts);
check_cmd_options([{Std, I} = H | T], Pid, State, PortOpts, OtherOpts) when
    Std =:= stderr, I =/= Std; Std =:= stdout, I =/= Std
->
    if
        ...
        is_function(I) ->
            {arity, 3} =:= erlang:fun_info(I, arity) orelse
                throw({error, ?FMT("Invalid ~w option ~p: expected Fun/3", [Std, I])}),
            check_cmd_options(T, Pid, State, [Std | PortOpts], [H | OtherOpts]);
        true ->
            throw({error, ?FMT("Invalid ~w option ~p", [Std, I])})
    end;
check_cmd_options([{group, I} = H | T], Pid, State, PortOpts, OtherOpts) when
    is_integer(I), I >= 0;
    is_list(I);
    is_binary(I)
->
    check_cmd_options(T, Pid, State, [H | PortOpts], OtherOpts);
check_cmd_options([{user, U} | T], Pid, State, PortOpts, OtherOpts) when
    (is_list(U) andalso U =/= "");
...
check_pty_opts(Pty) when is_list(Pty) ->
    case
        lists:filter(
            fun
                ({K, V}) when is_atom(K), (is_integer(V) orelse is_boolean(V)) ->
                    not check_pty_opt(K, V);
                (_) ->
                    true
            end,
            Pty
...
check_receive({Stream, Pid, Bin} = A, Orig, Got, Timeout, TestName, Line) when
    is_atom(Stream), is_integer(Pid), is_binary(Bin)
->
    receive
        A ->
            true;
...
exec_run_many_test_() ->
    ...
    Delay =
        case os:getenv("PID_SLEEP_SEC") of
            false -> 1000;
            Y -> list_to_integer(Y) * 1000
        end,
    N =
        case os:getenv("RUN_COUNT") of
            false -> 900;
            X -> list_to_integer(X)
        end,
    M = N * 2,
    {setup,
        fun() ->
            {ok, Pid} = dockerexec:start([{debug, Level}]),
            Pid
        end,
        fun(Pid) -> exit(Pid, kill) end, [
            {timeout, 200,
...
test_monitor() ->
    {ok, P, _} = dockerexec:run("echo ok", [{stdout, null}, monitor]),
    ?receivePattern({'DOWN', _, process, P, normal}, 5000).

test_sync() ->
    ?AssertMatch(
        {ok, [{stdout, [<<"Test\n">>]}, {stderr, [<<"ERR\n">>]}]},
        dockerexec:run("echo Test; echo ERR 1>&2", [stdout, stderr, sync])
    ),
    ?AssertMatch(
        {ok, [{stdout, [<<"\n">>]}]},
...
test_winsz() ->
    ...
    ok = dockerexec:winsz(I, 99, 88),
    ok = dockerexec:send(I, <<"\n">>),
    ?receiveBytes({stdout, I, <<"LINES=99 COLUMNS=88\r\n">>}, 3000),
    ?receivePattern({'DOWN', _, process, P, normal}, 5000),
    % can set size on run
    {ok, P2, I2} = dockerexec:run(
...
test_stdin() ->
    {ok, P, I} = dockerexec:run("read x; echo \"Got: $x\"", [stdin, stdout, monitor]),
    ok = dockerexec:send(I, <<"Test data\n">>),
...
test_stdin_eof() ->
    case os:find_executable("tac") of
        false ->
            ok;
        _ ->
            {ok, P, I} = dockerexec:run("tac", [stdin, stdout, monitor]),
            [
                ok = dockerexec:send(I, Data)
             || Data <- [<<"foo\n">>, <<"bar\n">>, <<"baz\n">>, eof]
            ],
...
test_std(Stream) ->
    ...
    {ok, _, I} = dockerexec:run("for i in 1 2; do echo TEST$i; sleep 0.05; done" ++ Suffix, [Stream]),
    ?receiveBytes({Stream, I, <<"TEST1\n">>}, 5000),
    ?receiveBytes({Stream, I, <<"TEST2\n">>}, 5000),

    Filename = temp_file(),
    try
        ?AssertMatch({ok, []}, dockerexec:run("echo Test" ++ Suffix, [{Stream, Filename}, sync])),
        ...
        ?AssertMatch(
            {ok, []}, dockerexec:run("echo Test2" ++ Suffix, [{Stream, Filename, [append]}, sync])
        ),
        ?AssertMatch({ok, <<"Test\nTest2\n">>}, file:read_file(Filename))
    after
...
test_cmd() ->
    ...
    ?AssertMatch(
        {ok, [{stdout, [<<"ok\n">>]}]},
        dockerexec:run("/bin/echo ok", [sync, stdout])
    ),
    ?AssertMatch(
        {ok, [{stdout, [<<"ok\n">>]}]},
        dockerexec:run(<<"/bin/echo ok">>, [sync, stdout])
    ...
    ?AssertMatch(
        {ok, [{stdout, [<<"ok\n">>]}]},
        dockerexec:run(["/bin/bash", "-c", "echo ok"], [sync, stdout])
    ),
    ?AssertMatch(
        {ok, [{stdout, [<<"ok\n">>]}]},
        dockerexec:run([<<"/bin/bash">>, <<"-c">>, <<"echo ok">>], [sync, stdout])
...
test_executable() ->
    ...
    ?AssertMatch(
        ...
        begin
            Res = dockerexec:run("ls", [sync, {executable, "00kuku00"}, stdout, stderr]),
    ...
    ?AssertMatch(
        {ok, [{stdout, [<<"ok\n">>]}]},
        dockerexec:run("echo ok", [sync, {executable, "/bin/sh"}, stdout, stderr])
    ),
    ...
    try
        ...
        ?AssertMatch(
            {ok, [{stdout, [<<"ok\n">>]}]},
            dockerexec:run(File, [sync, stdout, stderr])
        ),
        ?AssertMatch(
            {ok, [{stdout, [<<"ok\n">>]}]},
            dockerexec:run([<<"/bin/bash">>, <<"-c">>, File], [sync, stdout, stderr])
        )
    after
        ok = file:delete(File)
    end.
...
test_redirect() ->
    ?AssertMatch(
        {ok, [{stderr, [<<"TEST1\n">>]}]},
        dockerexec:run("echo TEST1", [stderr, {stdout, stderr}, sync])
    ),
    ?AssertMatch(
        {ok, [{stdout, [<<"TEST2\n">>]}]},
        dockerexec:run("echo TEST2 1>&2", [stdout, {stderr, stdout}, sync])
    ),
    ok.
...
test_redirect_stdin() ->
    ...
    ?AssertMatch(
        {ok, [{stdout, [<<"ttt\n">>]}]},
        dockerexec:run("cat", [{stdin, "/tmp/output.txt"}, sync, stdout])
    ),
    ?AssertMatch(
        {ok, [{stdout, [<<"ttt\n">>]}]},
        dockerexec:run("cat", [{stdin, <<"/tmp/output.txt">>}, sync, stdout])
...
test_env() ->
    ?AssertMatch(
        {ok, [{stdout, [<<"X-Y\n">>]}]},
        dockerexec:run("echo $XXX-$YYY", [stdout, {env, [{"XXX", "X"}, {<<"YYY">>, <<"Y">>}]}, sync])
    ).

test_kill_timeout() ->
    %{ok, _OldDebug} = dockerexec:debug(3),
    {ok, P2, I2} = dockerexec:run("trap 'echo Got signal' SIGTERM; sleep 15", [
        {kill_timeout, 1}, stdout, monitor
    ]),
    timer:sleep(200),
    dockerexec:stop(I2),
    timer:sleep(50),
    %dockerexec:debug(_OldDebug),
    ?receivePattern({'DOWN', I2, process, P2, normal}, 5000).

test_setpgid() ->
    % Cmd given as string
    {ok, P0, P} = dockerexec:run("sleep  1", [{group, 0}, kill_group, monitor]),
    {ok, P1, _} = dockerexec:run("sleep 15", [{group, P}, monitor]),
    {ok, P2, _} = dockerexec:run("sleep 15", [{group, P}, monitor]),
...
test_pty() ->
    ?AssertMatch(
        {error, [{exit_status, 256}, {stdout, [<<"not a tty\n">>]}]},
        dockerexec:run("tty", [stdin, stdout, sync])
    ),
    ?assert(
        case dockerexec:run("tty", [stdin, stdout, pty, sync]) of
    ...
    {ok, P, I} = dockerexec:run("/bin/bash --norc -i", [stdin, stdout, pty, monitor]),
    ok = dockerexec:send(I, <<"echo ok\n">>),
    receive
        {stdout, I, <<"echo ok\r\n">>} ->
    ...
    ok = dockerexec:send(I, <<"exit\n">>),
    ?receivePattern({'DOWN', _, process, P, normal}, 1000).

test_pty_echo() ->
    % without echo
    {ok, _, I} = dockerexec:run("echo started && cat", [
        stdin,
        stdout,
        {stderr, stdout},
        pty,
    ...
    ?receiveBytes({stdout, I, <<"started\r\n">>}, 5000),
    ok = dockerexec:send(I, <<"test\n">>),
    ?receiveBytes({stdout, I, <<"test\r\n">>}, 5000),
    ok = dockerexec:kill(I, 9),
    % with echo
    {ok, _, I2} = dockerexec:run("echo started && cat", [
        stdin,
        stdout,
        {stderr, stdout},
        pty,
...
test_pty_opts() ->
    ...
    % without echo
    {ok, P, I} = dockerexec:run("echo started && cat", [
        stdin,
        stdout,
        {stderr, stdout},
        {pty, [{echo, false}]},
    ...
    % with echo
    {ok, P2, I2} = dockerexec:run("echo started && cat", [
        stdin,
        stdout,
        {stderr, stdout},
        {pty, [{echo, true}]},
