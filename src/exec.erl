        ...
        case wait_port_exit(State#state.port) of
            ...
            S ->
                error_logger:warning_msg(
                    "~w - exec process terminated (status: ~w)\n",
                    [self(), S]
                )
        end
    catch
...
test_root() ->
    case os:getenv("NO_ROOT_TESTS") of
        false ->
            ...
            ?AssertMatch(
                {error, "Not allowed to run without restricting effective users" ++ _},
                exec:start([root, {user, "xxxx"}])
            ),
            ?AssertMatch(
                {error, "Not allowed to run without providing effective user " ++ _},
...
test_std(Stream) ->
    ...
    try
        ...
        ?AssertMatch(
            {ok, []}, exec:run("echo Test2" ++ Suffix, [{Stream, Filename, [append]}, sync])
        ),
        ?AssertMatch({ok, <<"Test\nTest2\n">>}, file:read_file(Filename))
    after
...
test_kill_timeout() ->
    ...
    {ok, P2, I2} = exec:run("trap 'echo Got signal' SIGTERM; sleep 15", [
...
test_pty() ->
    ...
    ?assert(
        case exec:run("tty", [stdin, stdout, pty, sync]) of
